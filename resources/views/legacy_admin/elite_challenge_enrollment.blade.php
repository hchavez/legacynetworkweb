...
@section('content')
    <div class="panel panel-headline">
        <div class="panel-heading">
            <h3 class="panel-title">Elite Challenge Signups</h3>
        ...
        <div class="panel-body">
            <table id="elite_challenge_table" class="table table-hover table-responsive display" cellspacing="0" width="100%">
                <thead>
                <tr>
                    <th></th>
                    <th>ID</th>
...
@section('scripts')
    <script>
        $(function () {
            $('#elite_challenge_table').on('submit', '.user_synergy_id_form', function (e) {
                e.preventDefault();
            ...
            $('#elite_challenge_table').on('click', '.payment_declined', function (e) {
                e.preventDefault();

                $.ajax({
                    url: '/users/ehc_payment_declined/' + $(this).data('id'),
                    type: 'POST',
                    dataType: 'json',
                    data: {},
                    headers: getAjaxHeaders(),
            ...
            var table = $('#elite_challenge_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ url('legacy_admin/dt/elite_challenge') }}',
                    data: function(d) {
...
//                    var data = table.row(this).data();
        ...
        function format ( d ) {
            ...
            var url = '{{ url('users/customer_application/') }}' + '/';
            return  '<form class="user_synergy_id_form" data-id="'+d.id+'">\n' +
    '                    <input type="hidden" name="_method" value="PUT">\n' +
    '                    <ol>\n' +
    '                        <li>\n' +
    '                            <a href="' + url + d.id +'" class="" target="_blank">Print PREFERRED CUSTOMER APPLICATION Form</a>\n' +
    '                        </li>\n' +
    '                        <li>\n' +
